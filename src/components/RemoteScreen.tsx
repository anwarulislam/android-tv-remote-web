...
import {
    Settings, Power, SkipBack, Rewind, Play, FastForward, SkipForward,
    ChevronUp, ChevronDown, ChevronLeft, ChevronRight,
    Home, VolumeX, Volume2, Volume1, Mic, Menu, ArrowLeft, Keyboard, Tv2,
    Send, X,
} from 'lucide-react';
    ...
    useEffect(() => {
        function onKeyDown(e: KeyboardEvent) {
            ...
            if (!meta && !e.shiftKey) {
                ...
                if (e.key === 'ArrowLeft') { e.preventDefault(); sendKey('LEFT'); return; }
                if (e.key === 'ArrowRight') { e.preventDefault(); sendKey('RIGHT'); return; }
                if (e.key === 'Enter') { e.preventDefault(); sendKey('ENTER'); return; }
                if (e.key === 'Escape' || e.key === 'Backspace') {
                    e.preventDefault(); sendKey('BACK'); return;
                }
                if (e.key === ' ') { e.preventDefault(); sendKey('PLAY_PAUSE'); return; }
            }

            if (e.shiftKey && !meta) {
                if (e.key === 'ArrowRight') { e.preventDefault(); sendKey('FF'); return; }
                if (e.key === 'ArrowLeft') { e.preventDefault(); sendKey('REWIND'); return; }
            }

            if (meta) {
                if (e.key === 'ArrowRight') { e.preventDefault(); sendKey('NEXT'); return; }
                if (e.key === 'ArrowLeft') { e.preventDefault(); sendKey('PREV'); return; }
                if (e.key === 'ArrowUp') {
                    e.preventDefault();
                    setVolume(v => v + 1);
                    sendKey('VOL_UP');
                ...
                if (e.key === 'm' || e.key === 'M') { e.preventDefault(); sendKey('MUTE'); return; }
                if (e.key === 'h' || e.key === 'H') { e.preventDefault(); sendKey('HOME'); return; }
                if (e.key === 'k' || e.key === 'K') { e.preventDefault(); openIme(); return; }
            }
        }
        ...
        <div className="flex justify-center items-center min-h-screen w-[min(100vw,420px)] mx-auto bg-zinc-950 p-4 select-none">
            ...
            >
                ...
                <div className="glass rounded-2xl px-3 py-3 flex items-center justify-between">
                    <button className="media-btn w-10 h-10 rounded-xl text-white/40 hover:text-white hover:bg-white/8 flex items-center justify-center cursor-pointer" onClick={() => sendKey('PREV')} title="Prev (⌘←)">
                        <SkipBack size={18} />
                    </button>
                    <button className="media-btn w-10 h-10 rounded-xl text-white/40 hover:text-white hover:bg-white/8 flex items-center justify-center cursor-pointer" onClick={() => sendKey('REWIND')} title="Rewind (⇧←)">
                        <Rewind size={18} />
                    </button>
                    ...
                    <button className="media-btn w-10 h-10 rounded-xl text-white/40 hover:text-white hover:bg-white/8 flex items-center justify-center cursor-pointer" onClick={() => sendKey('FF')} title="Fast Forward (⇧→)">
                        <FastForward size={18} />
                    </button>
                    <button className="media-btn w-10 h-10 rounded-xl text-white/40 hover:text-white hover:bg-white/8 flex items-center justify-center cursor-pointer" onClick={() => sendKey('NEXT')} title="Next (⌘→)">
                        <SkipForward size={18} />
                    </button>
                </div>
                ...
                <div className="glass rounded-3xl p-6 flex items-center justify-center">
                    <div className="relative w-[210px] h-[210px] sm:w-[220px] sm:h-[220px]">
                        ...
                        <button
                            ...
                            style={{ top: 8, left: '50%', transform: 'translateX(-50%)', width: 60, height: 60 }}
                            onClick={() => sendKey('UP')}
                            title="Up (↑)"
                        >
                        ...
                        <button
                            className="absolute flex items-center justify-center cursor-pointer bg-transparent border-none text-white/50 hover:text-white transition-all active:text-indigo-400 active:scale-90"
                            style={{ left: 8, top: '50%', transform: 'translateY(-50%)', width: 60, height: 60 }}
                        ...
                        <button
                            className="absolute flex items-center justify-center cursor-pointer bg-transparent border-none text-white/50 hover:text-white transition-all active:text-indigo-400 active:scale-90"
                            style={{ right: 8, top: '50%', transform: 'translateY(-50%)', width: 60, height: 60 }}
                        ...
                        <button
                            className="absolute flex items-center justify-center cursor-pointer bg-transparent border-none text-white/50 hover:text-white transition-all active:text-indigo-400 active:scale-90"
                            style={{ bottom: 8, left: '50%', transform: 'translateX(-50%)', width: 60, height: 60 }}
                ...
                <div className="flex gap-3">
                    ...
                    <div className="glass rounded-2xl flex flex-col items-center py-3 gap-1 min-w-[64px]">
                        <button
                            ...
                            onClick={() => { setVolume(v => Math.min(v + 1, volumeMax)); sendKey('VOL_UP'); }}
                            title="Volume Up (⌘↑)"
                        >
                            <Volume2 size={18} />
                        </button>
                        ...
                        <button
                            className="vol-btn w-10 h-10 rounded-xl flex items-center justify-center text-white/60 hover:text-white hover:bg-white/8 cursor-pointer"
                            onClick={() => { setVolume(v => Math.max(0, v - 1)); sendKey('VOL_DOWN'); }}
                            title="Volume Down (⌘↓)"
                        >
                            <Volume1 size={18} />
                        </button>
                    ...
                    <div className="flex-1 grid grid-cols-3 gap-2 sm:gap-3">
                        <IconBtn onClick={() => openIme()} title="Keyboard (⌘K)" className="glass min-h-[56px] text-white/55 hover:text-white hover:bg-white/10"><Keyboard size={18} /></IconBtn>
                        <IconBtn onClick={() => sendKey('HOME')} title="Home (⌘H)" className="glass min-h-[56px] text-white/55 hover:text-white hover:bg-white/10"><Home size={18} /></IconBtn>
                        <IconBtn onClick={() => sendKey('MUTE')} title="Mute (⌘M)" className="glass min-h-[56px] text-white/55 hover:text-white hover:bg-white/10"><VolumeX size={18} /></IconBtn>
                        <IconBtn onClick={() => sendKey('MIC')} title="Voice search" className="glass min-h-[56px] text-white/55 hover:text-white hover:bg-white/10"><Mic size={18} /></IconBtn>
                        <IconBtn onClick={() => sendKey('MENU')} title="Menu" className="glass min-h-[56px] text-white/55 hover:text-white hover:bg-white/10"><Menu size={18} /></IconBtn>
                        <IconBtn onClick={() => sendKey('BACK')} title="Back (Esc/⌫)" className="glass min-h-[56px] text-white/55 hover:text-white hover:bg-white/10"><ArrowLeft size={18} /></IconBtn>
                    </div>
                </div>
